...
for image in "${container_images[@]}"; do
    ...
    if [ ! -f "$DOWNLOAD_DIR_NVD_TAR"/"$image_name".tar ]; then
        vcf imgpkg copy -i "$image" --to-tar "$DOWNLOAD_DIR_NVD_TAR"/"$image_name".tar --cosign-signatures
    fi
done
